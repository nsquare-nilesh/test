...
class SJB_Admin_ListingImport_EditImport extends SJB_Function
{
	...
	public function execute()
	{
		...
		if (!empty ($_REQUEST ['xml']) || $parserInfo) {
			if ($parserInfo) { // load exist parser
				...
				if ($formIsSubmitted) {
					...
					if (!empty($mapped) && is_array($mapped) && !empty($original_xml) && empty($errors)) {
                        ...
                        \SJB\Analytics\Logger::log('Added XML Import');
		...
		if (empty($selectedProduct) && empty($errors)) {
			$errors[] = 'Please select a product';
		}

		if (!filter_var($pars_url, FILTER_VALIDATE_URL) && empty($errors)) {
			$errors[] = 'Please input correct URL';
		}
