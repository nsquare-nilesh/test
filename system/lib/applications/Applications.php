...
class SJB_Applications
{
	...
	public static function getByJob($listingID, $orderInfo = false, $limit = false)
	{
		...
		FROM `applications` `a`
			INNER JOIN `listings` l ON
				`l`.`sid` = `a`.`listing_id`
				{$order['join']}
				WHERE `a`.`listing_id` = ?s AND `a`.`hidden` = 0 {$order['order']} {$limitFilter}", $listingID);
		return $apps;
	}
	...
	public static function getByEmployer($userSID, $orderInfo, $limit = false)
	{
		...
		$apps = SJB_DB::query("
			...
			INNER JOIN `listings` l ON
				`l`.`sid` = `a`.`listing_id`
				{$order['join']}
			WHERE `l`.`user_sid` = ?s and `a`.`hidden` = 0 {$order['order']} {$limitFilter}", $userSID);
		return $apps;
	}
	...
	public static function create($listing_id, $jobseeker_id, $resume, $comments, $file, $mimeType, $file_sid, $post = false)
    {
        ...
        $res = SJB_DB::query("
            ...
            values(?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s)", $listing_id, $jobseeker_id ? $jobseeker_id : 0, $comments, SJB_DateType::mysqlNow(), $resume, $file, $mimeType, $jobSeekerName, $jobSeekerEmail, $file_id);
        return !empty($res);
    }
    ...
    public static function hide($id, $user)
    {
        $valid = SJB_DB::queryValue('select `a`.`id` from `applications` a
            inner join listings l on l.sid = a.listing_id
            inner join users u on u.sid = l.user_sid
            where a.id = ?n and u.sid = ?n', $id, $user);
        if ($valid) {
