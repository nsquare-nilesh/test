...
class SJB_Invoice extends SJB_Object
{
    ...
    {
        foreach ($this->getPropertyValue('items')['products'] as $product) {
            $product = SJB_ProductsManager::getProductInfoBySID($product);
    ...
    {
        return md5($this->getID() . serialize($this->getPropertyValue('items')['products']) . $this->getUserSID());
    }
}
